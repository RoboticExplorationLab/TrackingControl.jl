# TrackingControl functions taken from TrajOpt written by Brian
...
function get_control(cntrl::TVLQR, x, t)
    k = get_k(cntrl, t)
    zref = cntrl.Z[k]
    dx = RobotDynamics.state_diff(cntrl.model, x, state(zref))
...
#=                 #Original Function, removed for now
...
=#
function linearize(model::AbstractModel, Z::Traj) # Function Added for comptability with SizedDynamicsExpansion in expansions.jl used in TVLQR
    N = length(Z)
    n = length(Z.data[1]._x)  # added for next line
    m = length(Z.data[1]._u)  # added for next line
    D = [SizedDynamicsExpansion{Float64}(n,m) for k = 1:N]
    for k = 1:N
        _linearize!(D[k], model, Z[k])
    end
    A = [SMatrix(d.A) for d in D]
...
function _linearize!(D::SizedDynamicsExpansion, model::AbstractModel, z::AbstractKnotPoint)  # TO module before SizedDynamicsExpansion
    discrete_jacobian!(RK3, D.∇f, model, z)
	D.tmpA .= D.A_  # avoids allocations later
	D.tmpB .= D.B_
    x2 = discrete_dynamics(RK3, model, z)
    G1 = RobotDynamics.state_diff_jacobian(model, state(z))         # change that TO before state_diff_jacobain
    G2 = RobotDynamics.state_diff_jacobian(model, x2)               # change that TO
    #error_expansion!(D,G1,G2)                                      # removed for now
    return nothing
end
